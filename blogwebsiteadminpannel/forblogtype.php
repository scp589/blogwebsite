...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
              ...
              $result1 = mysqli_query($connect, $sql1);
                  ...
                  //end for fetching the the number of blogs
                  ...
                  // end of active link
                  echo '<li class="nav-item">
                  <a class="nav-link '.$active1.'" href="forblogtype.php?type='.$row['type'].'">'.$row['type'].'<span class="badge bg-secondary">'.$num.'</span></a>    
                </li>';
                }
              }
              else{
              ...
              if($result_for_latest){
                  // start of finding number for latest blog
                $num_for_latest = mysqli_num_rows($result_for_latest);
                $row_for_latest = mysqli_fetch_assoc($result_for_latest);
                // end of finding number for latest blog
                //satrt of active_link_for_latest
                $active_link_for_latest  = "";
                if(isset($_GET['latest'])){
                    if($_GET['latest']==$row_for_latest['latest']){
                        $active_link_for_latest = "active";
                    }
                }
                // end of active_link_for_latest
                echo '<li class="nav-item">
                <a class="nav-link '.$active_link_for_latest.'" href="forblogtype.php?latest='.$row_for_latest['latest'].'">latest
    ...
    <div class="container pt-3" id="myUL">
        ...
        <div class="row justify-content-end stack">
            <span class="my-span"><button class="btn btn-danger my-1" name="deletemulti"
                    type="submit">DELETE</button></span>
            <span class="my-span"><a class="btn btn-primary my-1" onclick="selectmulti()">Select multipule</a></span>
        </div>
        <?php
        include "partials/db.php";
        //start get data by type
        if(isset($_GET['type'])){
          $type_for_get = $_GET['type'];
          $sql1 = "SELECT * FROM `blog1` WHERE `blog1`.`type`= '$type_for_get'";
          $result1 = mysqli_query($connect, $sql1);
        if ($result1) {
        ...
        // end get data by type
        if(isset($_GET['latest'])){
          $latest_for_get = $_GET['latest'];
          $sql_for_latest = "SELECT * FROM `blog1` WHERE `blog1`.`latest`= '$latest_for_get'";
          $result_for_latest = mysqli_query($connect, $sql_for_latest);
        if ($result_for_latest) {
            while ($row_for_latest1 = mysqli_fetch_assoc($result_for_latest)) {
                echo '<div class="card mb-3">
                <h5 class="card-header">Posted on '.$row_for_latest1['dt'].'</h5>
                <div class="card-body">
                    <h5 class="card-title">'.$row_for_latest1['title'].'</h5>
                    <p class="card-text">'.substr($row_for_latest1['content'],0,400).'</p>
                    <a href="single.php?title='.$row_for_latest1['title'].'" class="btn btn-primary">Read more</a>
                    <span class="check-in"><input type="checkbox" value="'.$row_for_latest1['sno'].'" name="did[]"></span>
                </div>
            </div>';
            }
        }
